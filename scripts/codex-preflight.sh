...
usage() {
  cat <<'EOF'
Usage:
  scripts/codex-preflight.sh search --pattern <pattern> [--path <path>]
  scripts/codex-preflight.sh edit --file <path> [--tool Edit|Write|NotebookEdit]
  scripts/codex-preflight.sh bash --command <command>
  scripts/codex-preflight.sh repo --path <path>
  scripts/codex-preflight.sh policies [--cwd <path>]
...
Use a scoped path such as companies/, workers/, projects/, workspace/, or a
specific repo path.
EOF
      exit 2
      ;;
