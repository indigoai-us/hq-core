...
#
# Walks packages/*/package.yaml at session start (or on demand) and creates the
...
#   contributes.policies[name]   packages/{pkg}/policies/{name}.md    -> .claude/policies/{name}.md
#   contributes.scripts[name]    packages/{pkg}/scripts/{name}        -> scripts/{name}
#
...
HQ_ROOT="${HQ_ROOT:-$(pwd)}"
PACKAGES_DIR="$HQ_ROOT/packages"

warn() { printf '  [warn] %s\n' "$*" >&2; }
info() { [[ "${HQ_SCAN_QUIET:-0}" == "1" ]] || printf '  %s\n' "$*"; }
die()  { printf '[scan-packages] error: %s\n' "$*" >&2; exit 1; }

[[ -d "$PACKAGES_DIR" ]] || { info "[scan-packages] no packages/ dir; nothing to wire"; exit 0; }
  ...
  while IFS=$'\t' read -r key item; do
    ...
    case "$key" in
      workers)
        ...
        dst="$HQ_ROOT/workers/public/$item"
        ;;
      knowledge)
        src="$pkg_dir/knowledge/$item"
        dst="$HQ_ROOT/knowledge/public/$item"
        ;;
      skills)
      ...
      policies)
        ...
        dst="$HQ_ROOT/.claude/policies/$item.md"
        ;;
      scripts)
        src="$pkg_dir/scripts/$item"
        dst="$HQ_ROOT/scripts/$item"
        ;;
      *)
