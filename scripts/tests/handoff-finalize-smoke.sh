...
echo "secret" > settings/secret.json

out=$(bash scripts/handoff-finalize.sh \
  --title "Handoff: smoke" \
  --summary "Smoke test" \
  --message "Smoke test" \
  --next-steps-json '[]' \
  --files-touched-json '["tracked.txt", {"path":"notes/new.md","reason":"new note"}, {"path":"missing.txt"}, {"path":"../escape.txt"}, {"path":"settings/secret.json"}]' \
  --learnings-json '[]' \
  --tags-json '["test"]' \
  --slug "smoke")
...
if git cat-file -e HEAD:settings/secret.json 2>/dev/null; then
...
jq -e '.skipped_paths[] | select(.path == "settings/secret.json" and .reason == "sensitive-path")' "$changeset_path" >/dev/null \
  || fail "sensitive path skip not recorded"

rm tracked.txt
delete_out=$(bash scripts/handoff-finalize.sh \
...
summary=$(bash scripts/hq-status-summary.sh \
