...
#   sweep        Foreground sweep. Cheap-exit / cooldown / live-lock skip, else
#                spawn the detached bg sweep and return <1s. Fired by leftover
#                SessionStart leaf 36-work-mesh-sweep.sh on boxes where hq rescue
#                preserved the pre-v15.0.121 client (deleted from the release).
...
# ---------------------------------------------------------------------------
# Process-level single-flight for the SessionStart sweep.
# Per-(sid,slug) claims still serialize item work; without a process lock,
# every session start nohup's another jq scan of the whole spool (Mercer:
# 329 overlapping __sweep_bg__ processes, load ~106 on 2 vCPUs).
# ---------------------------------------------------------------------------
wm_sweep_timeout_sec()    { printf '%s' "${HQ_WORK_MESH_SWEEP_TIMEOUT_SEC:-60}"; }
wm_sweep_cooldown_sec()   { printf '%s' "${HQ_WORK_MESH_SWEEP_COOLDOWN_SEC:-300}"; }
wm_sweep_lock_stale_sec() { printf '%s' "${HQ_WORK_MESH_SWEEP_LOCK_STALE_SEC:-120}"; }

wmc_sweep_lock_dir()  { printf '%s/workspace/work-mesh/sweep.lock' "${WM_ROOT:-$HQ_ROOT}"; }
wmc_sweep_last_file() { printf '%s/workspace/work-mesh/sweep.last' "${WM_ROOT:-$HQ_ROOT}"; }

# Cheap: missing/empty spool, or no attempt/posted lines. No jq.
wmc_sweep_has_work() {
  local spf
  spf="$(wm_spool_file)"
  [ -f "$spf" ] && [ -s "$spf" ] || return 1
  grep -Eq '"event":[[:space:]]*"(attempt|posted)"' "$spf" 2>/dev/null
}

wmc_sweep_recently_ran() {
  local lastf last now age cool
  lastf="$(wmc_sweep_last_file)"
  [ -f "$lastf" ] || return 1
  last="$(tr -dc '0-9' < "$lastf" 2>/dev/null)"
  [ -n "$last" ] || return 1
  now="$(date +%s 2>/dev/null || echo 0)"
  age=$(( now - last ))
  cool="$(wm_sweep_cooldown_sec)"
  [ "$cool" -gt 0 ] 2>/dev/null || return 1
  [ "$age" -ge 0 ] 2>/dev/null && [ "$age" -lt "$cool" ] 2>/dev/null
}
...
wmc_sweep_lock_pid() {
  ...
  [ -d "$lock" ] || return 1
  pid="$(tr -dc '0-9' < "$lock/pid" 2>/dev/null)"
  [ -n "$pid" ] || return 1
  printf '%s' "$pid"
}

wmc_sweep_lock_is_live() {
  local pid
  pid="$(wmc_sweep_lock_pid)" || return 1
  kill -0 "$pid" 2>/dev/null
}

# Snapshot descendants while the parent is still alive. After TERM, orphans
# reparent to init and a second ps walk from the parent pid would miss them.
wmc_sweep_children_of() {
  local parent="$1" pid ppid
  [ -n "$parent" ] || return 0
  (ps -axo pid=,ppid= 2>/dev/null || ps -eo pid= -o ppid= 2>/dev/null || true) \
    | while read -r pid ppid; do
        [ "$ppid" = "$parent" ] && [ "$pid" -gt 1 ] 2>/dev/null && printf '%s\n' "$pid"
      done
}

wmc_sweep_descendants() {
  local parent="$1" kid
  [ -n "$parent" ] || return 0
  for kid in $(wmc_sweep_children_of "$parent"); do
    printf '%s\n' "$kid"
    wmc_sweep_descendants "$kid"
  done
}

wmc_sweep_signal_tree() {
  local target="$1" sig="${2:-TERM}" kid
  [ -n "$target" ] && [ "$target" -gt 1 ] 2>/dev/null || return 0
  for kid in $(wmc_sweep_descendants "$target") $target; do
    [ "$kid" -gt 1 ] 2>/dev/null || continue
    kill -s "$sig" "$kid" 2>/dev/null || true
  done
}

wmc_sweep_reap_tree() {
  local target="$1" kids kid
  [ -n "$target" ] && [ "$target" -gt 1 ] 2>/dev/null || return 0
  kids="$(wmc_sweep_descendants "$target")"
  for kid in $kids $target; do
    [ "$kid" -gt 1 ] 2>/dev/null || continue
    kill -s TERM "$kid" 2>/dev/null || true
  done
  sleep 1
  for kid in $kids $target; do
    [ "$kid" -gt 1 ] 2>/dev/null || continue
    kill -s KILL "$kid" 2>/dev/null || true
  done
}

wmc_sweep_try_process_lock() {
  local lock steal
  lock="$(wmc_sweep_lock_dir)"
  ...
  # Reclaim only when the recorded owner is dead or the pid file is missing.
  # A live owner keeps the lock; the timeout watchdog + EXIT trap must drop it.
  if wmc_sweep_lock_is_live; then
    return 1
  fi
  steal="$lock.stale.$$"
...
wmc_sweep_release_process_lock() {
  local lock pid
  lock="$(wmc_sweep_lock_dir)"
  pid="$(wmc_sweep_lock_pid 2>/dev/null || true)"
  if [ -n "$pid" ] && [ "$pid" != "$$" ]; then
    return 0
  fi
...
wmc_sweep_mark_ran() {
  local lastf
  lastf="$(wmc_sweep_last_file)"
  mkdir -p "$(dirname "$lastf")" 2>/dev/null || true
  date +%s >"$lastf" 2>/dev/null || true
}

wmc_sweep_cleanup() {
  local why="${1:-EXIT}"
  trap - EXIT HUP INT TERM
  if [ -n "${WM_SWEEP_WATCHDOG_PID:-}" ]; then
    wmc_sweep_signal_tree "${WM_SWEEP_WATCHDOG_PID}" KILL
    wait "${WM_SWEEP_WATCHDOG_PID}" 2>/dev/null || true
...
wmc_sweep_bg() {
  ...
  trap 'wmc_sweep_cleanup EXIT' EXIT
...
wmc_sweep_foreground() {
  ...
  wmc_sweep_has_work || return 0
  wmc_sweep_recently_ran && return 0
  wmc_sweep_lock_is_live && return 0
  local self logf
  self="$(wmc_self)"
  logf="$(wm_log_file)"
  mkdir -p "$(dirname "$logf")" 2>/dev/null || true
