...
wmc_sweep_try_process_lock() {
  local lock steal mt now age stale
  lock="$(wmc_sweep_lock_dir)"
  ...
  # Live owner: only steal once the lock is older than the stale window
  # (hung sweep whose timeout watchdog failed). Dead/missing pid: steal now.
  if wmc_sweep_lock_is_live; then
    stale="$(wm_sweep_lock_stale_sec)"
    mt="$(wm_file_mtime "$lock")" || return 1
    now="$(date +%s 2>/dev/null || echo 0)"
    age=$(( now - mt ))
    [ "$age" -ge "$stale" ] 2>/dev/null || return 1
  fi
  steal="$lock.stale.$$"
...
wmc_sweep_cleanup() {
  ...
  if [ -n "${WM_SWEEP_WATCHDOG_PID:-}" ]; then
    kill "${WM_SWEEP_WATCHDOG_PID}" 2>/dev/null || true
...
wmc_sweep_bg() {
  ...
  if [ "$sec" -gt 0 ] 2>/dev/null; then
    ...
    wmc_sweep_bg_body &
    body=$!
    ( sleep "$sec" >/dev/null 2>&1; kill -TERM "$body" 2>/dev/null; sleep 1; kill -KILL "$body" 2>/dev/null ) >/dev/null 2>&1 &
    wd=$!
