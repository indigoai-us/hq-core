## Migrating through the HQ core layout rollup — 2026-05-14

### TL;DR

Run `/update-hq`, then verify that your visible HQ root is reduced to the stable operating directories:
...
This is a larger layout migration than a normal patch release. The updater handles the shipped files, but operators with local custom scripts, bookmarks, shortcuts, sync conflicts, or documentation links should review the path changes below.
...
- Runtime journal/data placeholders under root `data/` were removed; project-scoped journals now live with their projects.
- Native session project capture was added with `.claude/hooks/auto-session-project.sh`, `.claude/hooks/native-plan-project-sync.sh`, `core/scripts/session-project.sh`, and tests.
- Single-company auto-startwork was added with `.claude/hooks/auto-startwork.sh` and tests.
- After-turn suggestion handling was added under `core/hooks/Stop/`.
- Several command, skill, setup, update, journal, handoff, and bridge scripts were updated to use the new core layout.
- Public release privacy gates were restored or widened: private tenant slug scan, `/Users/` path tripwire over `core/scripts`, and session marker path hardening.
- `core/core.yaml` remains the core metadata file and now points locked documentation paths at `core/docs/hq/`.
...
- `data/`
- root `projects/` for personal/HQ projects
...
The new native-session helpers write session state under `.claude/state/` and project artifacts under `personal/projects/` unless a company-scoped project is selected. Session identifiers are sanitized before becoming filenames.

The new after-turn suggestion hook lives at `core/hooks/Stop/50-after-turn-suggestions.sh`. If you maintain custom lifecycle hook allowlists, add `core/hooks/Stop/` to the set of expected shipped hook paths.
...
After update and sync cleanup, run these checks from the HQ root:
