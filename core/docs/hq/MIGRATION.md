## Release: v15.0.0-beta.6 — HQ-native meeting notes & signals

- promote 2026-06-04 (HQ-native meeting notes & signals): HQ now tells every session where a company's meeting notes and signals live and to check them FIRST instead of email or third-party notetakers. Adds two generic, multi-company read skills — `/meeting-notes` (reads `companies/{co}/sources/meetings/` via the HQ meeting bot store / `hq meetings`) and `/signals` (reads `companies/{co}/signals/`: decisions, action items, wins, risks, questions, commitments) — plus a charter capability line, a soft global policy `native-knowledge-first` (resolve native stores first; never default to email or third-party notetakers; steer turnkey activation when a store is empty), a canonical reference `core/knowledge/public/hq-core/native-knowledge-stores.md`, and a USER-GUIDE section. Preference resolution: `meeting_notes_source` defaults to `hq-native`; override per company at `companies/{co}/settings/knowledge/preferences.yaml`. Signals/ontology run on HQ cloud and will be HQ Pro-gated once billing ships (not yet — provisioned per-company today via `/designate-team`). **Operator action:** none — `/meeting-notes` and `/signals` are available after `/update-hq`.

## Release: v15.0.6 — block learned-rules in the charter (`/learn` never writes to `.claude/CLAUDE.md`; charter `## Learned Rules` section removed + replaced with a pointer; `protect-core` guard rejects learned-rule writes to `CLAUDE.md`/`AGENTS.md` even under the bypass)

- promote 2026-06-01 (block learned-rules-in-charter): completes the v15.0.4 `/learn` storage-routing work by closing the *other* charter leak — learned rules written directly into `.claude/CLAUDE.md`'s `## Learned Rules` section, which `/update-hq` replaces wholesale (so per-instance learnings were lost on upgrade AND re-shipped to every install as locked-scope drift). Shipped changes: (1) new hard policy `core/policies/learned-rules-never-in-claude-md.md` — learned rules live in policy files (`personal/policies/` for owner/universal, `companies/{co}/policies/`, or `core/policies/` for release-shipped), never the charter. (2) `.claude/skills/learn/SKILL.md` Step 6 "global promotion" is rewritten to raise a rule's enforcement to `hard` in a policy file instead of copying it into CLAUDE.md; the legacy CLAUDE.md `## Learned Rules` target, the 20-rule cap, and the "both target file AND CLAUDE.md" rule are removed. (3) `.claude/hooks/protect-core.sh` gains a guard that fires BEFORE the broad `HQ_BYPASS_CORE_PROTECT` bypass and rejects any Write/Edit to `CLAUDE.md`/`AGENTS.md` whose added content carries a learned-rule signature (`<!-- user-correction`, `<!-- back-pressure-failure`, `## Learned Rules`, or a `**NEVER/ALWAYS**: … <!--` bullet); benign charter edits and the wholesale `/update-hq` cp path are unaffected. (4) the charter's `## Learned Rules` section is removed and replaced with a pointer to where rules live; the prior entries already have dedicated policy files (new `core/policies/hq-load-company-hard-policies-on-mid-session-bind.md` captures the one that lacked a home). (5) `core/core.yaml` `rules.exclude` gains machine-local runtime artifacts (`.claude/launch.json`, `.claude/scheduled-tasks/`, `.claude/scheduled_tasks.lock`) and builder/admin-only `hq-pack-admin` tooling (`core/scripts/promote-hq-core-*.sh`, `.claude/kit/`, `.claude/audit-rules.yaml`, `.claude/scrub-denylist.yaml`) so they stop showing as base-release core drift. **Operator action:** after `/update-hq`, restart Claude Code once so the updated `protect-core.sh` loads; existing charter `## Learned Rules` entries are superseded by their policy files.
