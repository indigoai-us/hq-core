...
### Fixed — Work Mesh tool-writes hook preserves session binding (2026-09-21)
- **Work Mesh: the tool-writes hook no longer wipes a session's company, project and task binding on every edit.** `core/hooks/PostToolUse/35-work-mesh-tool-writes.sh` rewrote `~/.hq/work-context/sessions/<sid>.json` with a fixed five-field stub on every Edit/Write/Bash redirect, dropping `companyUid`, `companySlug`, `projectId`, `taskId`, `startedAt`, `bindingEpisodeId`, `decision` and the CLI's `contextStatus`. On macOS/Linux the next reconcile re-established the company (attribution flickered); on Windows nothing repaired it and attribution was lost for the session. The hook now calls `work_mesh_live_bump_tool_writes` from `core/scripts/lib/work-mesh-live-hook.sh`, which changes only `toolWrites` and `updatedAt` (jq when available, pure-bash in-place edit otherwise), creates the same minimal stub when the file is absent, leaves a file it cannot parse untouched, and keeps temp+mv and mode 600. Because hq-cli writes the same file without a lock (tmp + rename), the helper takes a `<path>.lock` mkdir lock (bounded ~200ms wait, stale reclaim after 5s; bump skipped if the lock cannot be taken) and re-reads the file right before its rename so a reconcile write landing mid-bump is merged instead of overwritten.
- Regression suite: `core/scripts/tests/work-mesh-tool-writes-preserve.test.sh` (seeded compact/pretty state with and without jq, absent-file stub, missing counters, malformed JSON, contextStatus bound, concurrent writer mid-bump, held and stale locks; picked up by the `work-mesh-tests` pr-checks job).
...
- Regression suite: `core/scripts/tests/block-env-dump.test.sh` (hook-gate + master-hook under minimal/standard/strict; wired into `pr-checks`).

### Fixed — shipped `permissions.defaultMode` is `auto` again (2026-09-18)
- **v15.0.146 changed the shipped default permission mode from `auto` to `plan` without notice.** The change was one line inside an unrelated hooks fix (`020f891f`) and was not intended. Every `hq rescue` or `/update-hq` run since then reset operators to Plan mode, and PR #772 added a healer step that rewrote `auto` back to `plan` on every run. This restores `auto` in `.claude/settings.json`, removes the healer rewrite so `restore-hook-settings.sh` leaves `defaultMode` alone, and flips the release contract check to require `auto`. Policy `hq-claude-code-default-mode-plan-not-auto` is rewritten (v3) to document `auto` as the shipped default, forbid healers from touching `defaultMode`, and drop the advice to set `auto` in `settings.local.json`, which Claude Code ignores. Operators who were put into Plan mode: run `/update-hq` once, or switch the mode picker in the Claude desktop app.
- **v4 of that policy names the Auto hatch that local settings still ignore.** Claude Code does not honor `defaultMode: "auto"` from project or `.claude/settings.local.json`. If Plan still returns after `/update-hq`, set `permissions.defaultMode` to `"auto"` in `~/.claude/settings.json`, or use the mode picker. Do not tell operators that the local file alone restores Auto.

### Fixed — Browser MCP coordinate clicks on Google Docs canvas (2026-09-18)
- **Do not click canvas apps from screenshot coordinates.** Claude-in-Chrome `computer:left_click` aimed from a reported screenshot frame (1277x952) landed ~30% low/right on the Google Docs canvas. Policy `hq-prefer-agent-browser` v4, `core/knowledge/public/agent-browser/`, and the PreToolUse MCP nudge now require snapshot/find refs for DOM controls and keyboard for canvas text.
- **Unscoped `type` can write into the live document body.** A find-box type inserted stray text into a team Doc and autosaved. Confirm focus with a DOM ref or a keyboard-opened control before typing.
- **agent-browser cannot reuse a logged-in Chrome Google session.** Google Workspace needs headed sign-in, then `state save`. The MCP fallback hook now warns even when agent-browser is missing (the path that produced the mistype). Regression: `core/scripts/tests/prefer-agent-browser-canvas.test.sh` (run from the `prefer-native-capabilities` pr-checks job).

### Fixed — update-hq / hq rescue no longer leaves hook wiring only in settings.local.json (2026-09-18)
- **Empty `.claude/settings.json` hook arrays after `/update-hq` are restored.** Rescue can treat shipped `master-hook.sh` lines as local drift, write empty event arrays into `settings.json`, and copy the dispatchers into `settings.local.json`. Deleting that local `hooks` key then disabled every hook. `core/scripts/restore-hook-settings.sh` writes the nine canonical master-hook registrations back into `settings.json` (PATH and other keys stay), and removes the local `hooks` key. `/update-hq` and setup run it after rescue; `check-hq-hooks.sh` fails when the local overlay still carries `master-hook.sh`.
- Regression suite: `core/scripts/tests/restore-hook-settings.test.sh` (run from `hook-settings-release-contract.test.sh` in pr-checks).

### Fixed — Handoff/learn/checkpoint chat reports follow plain-language output (2026-09-18)
- **`/handoff`, `/learn`, and `/checkpoint` no longer paste operator Report fields when the default HQ output style is on.** Those skills' chat templates printed Scope, Dedup, Action, paths, thread IDs, and PIDs even after a successful save (feedback 2286). Default HQ now ends with a short plain line; `/output-style hq-operator` still gets the technical report. Files those commands write stay full prose.
- Regression suite: `core/scripts/tests/skill-plain-report.test.sh` (run from `skill-command-refs.test.sh` in pr-checks).

### Fixed — Stale workers registry no longer advertises missing directories (2026-09-17)
- **Active `core/workers/registry.yaml` rows whose path is gone are called out at SessionStart.** A registry entry that points at nothing is discoverable (Read/grep the index) but unusable, which routed an Amass depletions session to a raw ingest script instead of the missing CHANGES worker (feedback_a0ca0cb2). `inject-local-context.sh` now lists those ids and tells the session not to treat them as available. Helper: `core/scripts/lib/workers-registry.sh`.
- **`/hq-sync` regenerates the workers registry after a completed sync.** `hq reindex` is path-gated on in-session edits and does not run after an out-of-band pull, so the index could stay stale when worker directories were never downloaded or were pruned. `/run` and `/startwork` skip missing paths instead of inventing a fallback.
- Regression suite: `core/scripts/tests/workers-registry-missing.test.sh` (runs from the `manifest-nested-parse` pr-checks job).
...
### Fixed — `claude-bg-shell-pressure-reap` v2: detach recipe that detaches, narrower rule (2026-09-15)
- **The detached-waiter recipe now actually detaches.** v1 recommended `setsid nohup <deadline> <script> & … disown`. On Linux, launched with `&` from the tool shell, `setsid` is not a process-group leader, so it calls setsid(2) in place and stays the shell's child (`ppid` is the shell, not 1). The recipe is now `setsid --fork sh -c 'd=$1; shift; echo $$ >"$d/poll.pid"; exec <deadline> "$@"' _ "$dir" <command> [args...]`. It forks into a new session, clears any stale PID file first, passes every argument through, waits (bounded) for the PID file because `setsid --fork` returns before the child writes it (missing on 8 of 20 immediate checks), and requires `ps -o ppid=` to no longer be the launching shell (usually 1; a systemd user subreaper can also adopt it).
- **Narrower, stronger rule 3.** Nine hours after v1 shipped, sessions that received it kept idle-waiting with harness-tracked background loops, because v1 only forbade re-arming after a kill. v2 forbids idle-waiting on external state (logs, CI, deploys, lanes) with a harness-tracked background task. Long builds and test suites that do real work stay allowed as background tasks, consistent with `hq-foreground-timeout-killed-by-harness-deadline`.
- `claude-bg-shell-pressure-reap-trigger.test.sh` now executes the documented recipe verbatim, with a short deadline and a multi-argument command. It asserts the PID file appears, `ppid` is 1, and every argument arrives. It still rejects the in-place `setsid nohup` form.

### Added — Policy: background Bash tasks killed "because the system is running low on memory" (2026-09-15)
- **New hard policy `claude-bg-shell-pressure-reap`.** Idle `run_in_background` Bash pollers were being killed dozens of times with this message while `free` showed 34 GB available. The killer is Claude Code itself, not kernel OOM, cgroups, or oomd: its bundled Bun runtime arms a PSI trigger (`some 150000 2000000` on `/proc/pressure/memory`), and the CLI kills tracked background shells in sessions idle for 30+ minutes when that trigger fires. The policy explains how to diagnose it (PSI and refaults, not `free`), forbids re-arming the same loop, points to pollers that survive (Monitor, or a detached process with a wake path), and documents the operator-only `CLAUDE_CODE_DISABLE_BG_SHELL_PRESSURE_REAP` opt-out. It fires on any harness-tracked background Bash call (`run_in_background`, surfaced at most once per session by the injector's dedupe ledger) and on prompts about low-memory or OOM kills of background tasks. To support that, `derive-trigger-facts.sh` now emits a structured `run_in_background` fact for PreToolUse Bash calls with `run_in_background: true`, so policies can tell a backgrounded command from the same command run in the foreground. Regression suite: `core/scripts/tests/claude-bg-shell-pressure-reap-trigger.test.sh`.
...
- `work-mesh-live-hooks.test.sh` no longer flakes on loaded CI runners. The
  ...
  is under half the CPU count and the bash-true baseline p50 is at most 10ms.
  `cpu_p95 < 20ms` (user+sys) remains asserted everywhere;
  `HQ_WORK_MESH_FORCE_WALL_ASSERT=1` forces the wall assertion. Motivated by
  PRs #698 and #701 failing with `adj_p95=229ms` at load 0.72/2 on a 2-CPU
  GitHub runner while the same job passed on re-run.

### Fixed — Safety hooks (CEEL self-audit follow-up, 2026-09-07)
- **`context-warning-50.sh` is retired for real.** The 50% context Stop hook printed a mandatory checkpoint directive to stdout, but Claude Code does not deliver Stop-hook stdout to the model, so the directive was never seen (confirmed on a session that fired it at 10:20 and ran until 13:27 unaware). Its threshold was also a transcript-size heuristic (1.75 MB with the default window), uncalibrated against real usage. The July 2026 recalibration (PR #436) retired it, but that PR was closed by an age-based backlog sweep and the local deletion was restored by sync, leaving the hook registered. It is now unregistered from `.claude/settings.json` and `hook-gate.sh`, left in place as an inert stub so stale references no-op, and checkpointing keeps its one mechanical trigger: PreCompact (`auto-checkpoint-precompact.sh`), whose output the model does receive. `core/scripts/tests/context-checkpoint-hooks.test.sh` now pins the retirement (silent at any size, not registered anywhere) alongside the PreCompact directive.
- **`detect-secrets.sh` scans file writes, not just Bash commands.** Since March the detector guarded only `Bash`, and three policies documented "write it with the Write tool instead" as the sanctioned recovery from a false positive — so a real secret written into a file under `companies/` (synced to the vault and to teammates) was never scanned. The same patterns and the same false-positive escapes now apply to `Write` content, `Edit` / `MultiEdit` `new_string`, and `NotebookEdit` `new_source`; the block message names the tool and points at the vault workflow. Example keys in docs and tests must look fake or be assembled from fragments. Regression suite: `core/scripts/tests/detect-secrets-file-tools.test.sh` (new, wired into `pr-checks`).
- **`mandatory-scope-authorizer.sh` covers file mutations.** The cross-company guard was registered for `Read`, `Grep`, `Glob`, and `Bash` only, so a session bound to one company could write into another tenant's folder with the editing tools while the identical path was blocked in Bash. `Write`, `Edit`, `MultiEdit`, and `NotebookEdit` now go through the same session-identity and path rules; read-side behaviour is unchanged and deliberately strict (see the hook header for the 2026-08-19 incident). Regression coverage: cases `[24]`–`[26]` in `core/scripts/tests/mandatory-scope-authorizer.test.sh`.
### Changed — Policy injection is an index with retrieval on demand (2026-09-07)
- **No more count cap; every matching policy is listed.** `inject-policy-on-trigger.sh` now emits one index line per matching policy (id, tier, scope, one-line summary) with no 16-slot cut. Only HARD rules that matched *this* event (reactive) carry full text, inside the 5 KB budget; always-on baseline rules are index lines too. The whole emission stays under the 8,000-byte host ceiling, and the trailer tells the agent to pull a rule's full text (`qmd get <slug>` or the file) before acting in its area. `HQ_SESSION_POLICY_CAP=<n>` restores the legacy count cap for box preflight. Rationale: one session fired 1,466 policies through triggers that match on any word, so body injection could never fit the pipe; an index of all matches plus retrieval of the few that matter is the model Claude Code, Cursor rules, and skills already use.
- **Ranking by specificity.** Within each reactive/baseline and hard/soft group, a policy whose `when:` matched more distinct facts (`deploy && vercel && indigo`) ranks above one that matched fewer (`deploy`); scope order is the tie-break.
- **Retrieval is now measured.** New PostToolUse hook `record-policy-retrieval.sh` (Read + Bash) records when a session pulls a policy's full text into `workspace/orchestrator/policy-retrieval-state/<session>.txt`. `policy-age-report.sh` exposes it as `retrieved`, the usage signal retirement should key on.
- **Retirement is a state, not a deletion.** `core/scripts/policy-retire.sh <slug> --reason …` sets `status: retired` (with who/when/why) and `--restore` reverses it; `--from-report <age-report.json> --class <never-fired|dormant|stale-refs|single-incident-old> --yes` retires a whole candidate class. The loader, the company-bind digest, and the age report all skip retired policies. The loader also skips HQ Sync conflict twins (`*.conflict-*`) by filename.
- **A delivery benchmark exists.** `core/scripts/policy-benchmark.sh live` reports truncated hook outputs, sessions, policies fired and retrieved per session, and the retrieval rate for the last N days; `policy-benchmark.sh scenarios --file <json>` runs the loader against scenario prompts and scores recall, full-text delivery for hard rules, and bytes under the ceiling. Behavioural compliance (did the agent obey) still needs live agent runs and is the follow-up.
- **Core change tests are written down and include live runtimes.** `core/docs/hq/CORE-CHANGE-TESTS.md` lists the four layers: CI unit/contract suites, the new `core/scripts/tests/harness-live-smoke.sh` (one real headless turn each in Claude Code, Codex, and Grok Build, asserting hooks fired and every reminder stayed under the host ceiling via the new per-event `workspace/orchestrator/policy-emit-stats` ledger), the agents-v2 fleet canary drill (`--fleet`), and the delivery benchmark trend.
- **Retirement mechanism hardened after its first real run.** Only `core/`, `.claude/`, `companies/<co>/` references are judged (personal/, workspace/, repos/ are environment-specific); runtime dirs and placeholder names are ignored; a same-stem sibling (`foo.sh` → `foo.mjs`) is a rename and a needs-fix, lines that say a path is deleted/legacy/not-to-be-used are not dependencies, and a rule is retired for staleness only when every judged reference is gone. `policy-retire.sh --auto` (human on the loop) writes one dated report per day, appending a section per run, and `/hq-checkup` runs it.
- Removed the last "context feels heavy / context is precious / session feels long" prose (remnants of the closed July recalibration PR #436) from `/checkpoint`, `/setup`, and the auto-checkpoint spec; PreCompact is the only checkpoint trigger.


### Changed — Policy delivery: ranked, under the host ceiling, and measurable (CEEL self-audit follow-up, 2026-09-07)
- **Every policy reminder now fits under Claude Code's hook-output ceiling.** Claude Code persists hook stdout above ~10,000 bytes to a file and shows the model a ~2 KB preview; the rest is lost for that turn. One install had 3,341 such truncated outputs, most from `inject-policy-on-trigger.sh` (hard-rule full-text budget 16 KB) and the company-bind digest in `hq-session.sh` (40 KB). Defaults are now 5,120 / 2,048 bytes per policy for the loader and 7,168 for the digest, and the loader enforces an absolute `HQ_POLICY_OUTPUT_CEILING_BYTES` (8,000) on its whole emission: over it, hard policies fall back to summary lines with a notice; still over, lower-ranked lines are cut and counted. Ledger recording moved ahead of emission so the fallback never double-records. `core/scripts/tests/inject-policy-output-ceiling.test.sh` fails if any default is raised past the ceiling.
- **Hard rules rank ahead of soft ones under the count cap.** The 16-slot cap cut in glob order, so a hard rule about production credentials could lose its slot to a soft style note that sorted earlier. Within each reactive/baseline group, `enforcement: hard` is now stable-partitioned first; scope order (company > repo > personal > core) is preserved inside each tier.
- **`core/scripts/policy-age-report.sh` gives rule retirement its inputs.** Per policy: sessions fired (from the trigger ledgers), last fired, age, backtick-quoted HQ paths in the rule that no longer exist (an inaccuracy proxy), and single-incident provenance. `--candidates` lists never-fired, dormant, stale-reference, and old single-incident rules with the evidence. It deletes nothing; it is the measurement half of "retire when out of date, fade when unused". Regression suite: `policy-age-report.test.sh`.

### Added — Status truth: done-marks and delegations that can be checked
- **Done-marks carry verified evidence.** The `/run-project` and `/execute-task` return contract gains an `evidence` list (what the worker actually produced: `path:`, `repo-path:`, `branch:`, `url:`; `commits` count automatically). `core/scripts/verify-story-deliverables.sh` checks every claimed reference (and any `deliverables` the PRD optionally declared up front) against disk and git, exits 3 naming anything that does not exist, and with `--write` records the verified references on the story as `evidence[]` — an audit trail that can be re-checked later, which is what the CEEL audit lacked. Deliberately not strict: a story with no evidence and no declaration still passes (reported as unverified); only a claim or declaration that turns out to be absent blocks the flag. Wired into `/run-project` Step 3b.7, `/execute-task` 7a.2, and `/prd` (optional field). Suite: `verify-story-deliverables.test.sh`.
- **Delegation ends in a receipt or a FAILED state.** `core/scripts/hq-delegate-pickup.sh` adds `picked-up` (an acknowledgement, a recipient commit on the handed-over branch, or operator-observed evidence) and `failed` (no evidence within `HQ_DELEGATE_PICKUP_WINDOW_HOURS`, default 72, with the reason). `/delegate` step 10 records it; `/startwork` probes manifests still at `sent`. Suite: `hq-delegate-pickup.test.sh`.
- **Handoff next steps carry ids and can be closed.** `handoff-finalize.sh` normalizes every step to `{id: "<thread>#<n>", status: "open"}`; `core/scripts/handoff-open-steps.sh list|close|reopen` lists open steps across recent handoffs and closes them in place. `/handoff`, `/resumework`, and `/startwork` use it so a step is either closed or still open — never silently re-copied forward. Suite: `handoff-open-steps.test.sh`.

### Added — hq-pack-engineering absorbed into the base release
- **The engineering surface ships in core again, reversing the v15.0.0 extraction.** 24 skills (`/tdd`, `/review`, `/execute-task`, `/architect`, `/diagnose`, `/investigate`, `/run-project`, `/land`, `/ship`, `/prd`, `/deep-plan`, and the rest), 6 workers (`qa-tester`, `security-scanner`, `accessibility-auditor`, `frontend-designer`, `performance-benchmarker`, `site-builder`), 4 knowledge sets (Ralph, agent-browser, ai-security-framework, dev-team), 3 policies (`e2e-testing-standards`, `hq-bugfix-requires-tests`, `hq-no-test-shortcuts`), and 3 hooks now ship with hq-core. Nothing needs installing: a lean greenfield install gets the full engineering surface, and the pack-install lines previously printed by `/plan`, `/ideate`, `/strategize`, and `/tutorial` are gone because the commands always resolve. Rationale: core's own charter already treated these guardrails as non-negotiable, so gating them behind an optional install left core asserting rules it did not ship.
- **`hq-pack-engineering` is no longer a recommended package** and has been dropped from `core/core.yaml`. Its `auto_install` conditional (the v15.0.0 transparent-extraction gate for upgraders) went with it, along with the test that covered only that predicate.
- **Upgraders: remove the now-redundant pack.** `/update-hq` deliberately preserves `core/packages/` across an upgrade, so the old pack is **not** removed for you and will sit alongside the native copies. `scan-packages.sh` refuses to clobber the shipped files, so nothing breaks — but clean it up once with:
  ```bash
  hq packs uninstall hq-pack-engineering
  ```
- **Defects fixed in transit**, all caught by core contracts the pack was never checked against: `/land` and `/land-batch` shipped `agents/openai.yaml` without the required `interface:` mapping; six skills declared `allowed-tools` that did not cover the concrete commands they document; `/outpost-host` invoked `host-app.sh` through its old `core/packages/...` path, which ceases to exist on uninstall; `/run-pipeline` documented commands prefixed with a hardcoded `cd /Users/{your-name}/Documents/HQ`; and `/review` pointed users at `/pr`, a command that ships nowhere.
- **`core/policies/hq-prefer-agent-browser.md` was NOT taken from the pack.** Core's copy is newer (hard enforcement, tier 1, the auto-install product direction); the pack carried a stale soft fork. The pack's `knowledge/agent-browser/auth-profiles.md` **was** taken — it adds the vault-backed `hq secrets exec` patterns and drops a hardcoded password from the worked example.
- **`hq-pack-policies-excluded-from-core-release` is narrowed, not retired.** It still keeps builder/admin-only content (`hq-pack-admin`) out of the base release, which `core/core.yaml` depends on, but now distinguishes that from an adopted capability pack, which may be promoted. This promotion is recorded as the worked precedent.

### Fixed — Active-run coordination
- **`block-on-active-run.sh` again consults the repo-run registry under the resolved root.** Since v15.0.112-beta.3 (commit `7f4ac726`), which added shell-command parsing, the hook sourced its helper library with `. "$HQ_ROOT/core/scripts/hook-lib.sh" 2>/dev/null || true`. Sourcing a **missing** file is fatal in a non-interactive bash — the `|| true` guard is bypassed — so on any resolved root lacking `core/scripts/hook-lib.sh` (fresh installs, git worktrees, or a `CLAUDE_PROJECT_DIR` pointing at a non-standard tree) the PreToolUse hook exited `rc=1` before reaching the ownership check, silently disabling cross-session Edit/Write/Bash blocking. The source is now guarded by a `[ -r ... ]` readability test so a missing lib degrades gracefully instead of aborting the hook. Regression coverage: case `[6]` of `core/scripts/tests/hook-path-resolution.test.sh` (fake root without `hook-lib.sh`) now also asserts a clean `rc=0`, pinning the source as non-fatal.

### Fixed — Autosave and handoff durability
- **HQ autosave and the handoff finalizer no longer fail silently (feedback_b16e277b).** `hq-autocommit.sh` shipped with `|| exit 0` on every git call and sent all output to `/tmp`, so an orphaned `.git/index.lock` stopped autosave for four hours while producing no signal anywhere — which read as a healthy quiet system. Git failures are now recorded in `workspace/logs/hq-autocommit.log` and warned about once per session per cause; deliberate no-ops (nothing staged, excluded path, live lock contention) stay silent as before, and a failure still exits 0 so it never blocks the editor (`HQ_AUTOCOMMIT_STRICT=1` makes it exit non-zero for diagnostics). A lock older than five minutes is called out as stale rather than treated as ordinary contention. `handoff-finalize.sh` used to emit `hq_committed:false` inside an exit-0 payload with the staged paths listed as `committed_paths`; it now reports `hq_commit_status` (`committed` / `nothing-to-commit` / `failed` / `stage-failed`), lists `committed_paths` only when the commit actually landed, carries git's message in `hq_commit_error`, reports unstageable paths in `stage_failures`, and exits **4** whenever the handoff is not durable — including the held-`index.lock` shape where every `git add` fails, nothing is staged, and no commit is even attempted. Regression suites: `core/scripts/tests/hq-autocommit.test.sh` (extended) and `core/scripts/tests/handoff-finalize-commit-status.test.sh` (new), both newly wired into `pr-checks`.

### Fixed — Agent runtime
- **Codex fleet reply capture (AE incident 2026-07-28).** codex-cli 0.145.0 can end a run with no final agent message on stdout (long runs finishing on hook/auto-checkpoint activity), which the session engine read as "no reply" — completed answers were discarded and users saw a task-failed message. The codex adapter now passes `--output-last-message` on both `codex exec` and `codex exec resume`, and `hq-agent-session.sh` resolves the reply through a fallback chain: provider stdout → `last-message.txt` → the last stderr `codex` block (codex-only, last resort; a genuinely empty run still fails honestly). Regression suite: `core/scripts/tests/hq-agent-session-last-message-fallback.test.sh`.
- **`check-hq-hooks.sh --require-ledger` is now runtime-aware for agents-v2 (hermes) fleet boxes (A5-SMOKE / live drill 2026-09-04).** The ledger assertion is unchanged — a present `workspace/orchestrator/policy-trigger-state/<session>.txt` still reads `OBSERVED` (exit 0), a missing one `NOT OBSERVED` (exit 2), same exit codes and same `hq doctor`-compat mapping. What changed is the recovery guidance: an agents-v2 box writes that SAME ledger through the SAME `.claude` hooks (dispatched by the on-box `hq-agents-v2-hook-adapter.sh`), but only during a live hermes turn, so a box that has taken no turn yet legitimately reads NOT OBSERVED. The checker now detects such a box (runtime marker `${HQ_RUNTIME_MARKER_FILE:-/var/lib/hq-agent/runtime.json}` `runtimeMode=="agents-v2"`, or the adapter installed under the tree at `.agents-v2-hooks/hq-agents-v2-hook-adapter.sh`) and prints v2-appropriate steps (drive one live Slack/`hq dm` turn, then re-run) instead of the Claude Desktop/SDK steps that misled operators into thinking hooks were broken. Regression coverage: new case `[18]` in `core/scripts/tests/hook-health-check.test.sh` (v2 guidance on both detection signals, identical OBSERVED/NOT OBSERVED assertion, no Claude-CLI leak, default runtime unchanged). The companion adapter-side proof lives in hq-agents-v2 (`hooks/tests/test-agents-v2-hook-adapter.sh`).
- **`check-hq-hooks.sh --require-ledger` now self-attests OBSERVED on an agents-v2 box under the `hq doctor` path (A5-SMOKE follow-up, live drill 2026-09-05).** The previous fix only made the *guidance* runtime-aware; the attestation still keyed on `hq doctor`'s `hooks.runtime.enforcement` verdict, which classifies the hermes host as platform "unknown" and refuses to self-attest. On a real HQ tree (where `hq` is present and the checker delegates to `hq doctor`) a v2.17 canary therefore read `NOT OBSERVED` (exit 2) even with a fresh, populated `workspace/orchestrator/policy-trigger-state/<session>.txt` and a fully wired `.claude/settings.json` — the inline fallback already reached OBSERVED on a present ledger, but the doctor path relayed the host-unknown FAIL verbatim. `render_from_doctor` now grants the identical OBSERVED verdict when, and only when, all three hold: the runtime is agents-v2 (`hq_runtime_mode` — runtime marker or the on-box adapter under the tree), `.claude/settings.json` wires `hq-agents-v2-hook-adapter.sh`, and a policy-trigger ledger exists (the exact session's under `--session-id`, otherwise any ledger modified within `HQ_V2_LEDGER_MAX_AGE_HOURS`, default 24h, so a long-dead tree cannot self-attest off a stale file). No non-agents-v2 host changes verdict and every Claude-CLI path stays byte-identical. Regression coverage: new case `[19]` in `core/scripts/tests/hook-health-check.test.sh` drives the doctor path with a stub `hq` reproducing the exact host-unknown verdict (wired v2 + fresh ledger → OBSERVED/exit 0; wired v2 + missing ledger → NOT OBSERVED/exit 2 with v2 guidance; unknown host + ledger → unchanged NOT OBSERVED; stale ledger gated without `--session-id` but self-attested with it).
- **agents-v2 attestation signal 2 now keys on the hermes runtime config, not `.claude/settings.json` (A5-SMOKE follow-up, live drill 2026-09-05).** The prior `[19]` fix asserted `.claude/settings.json` wires `hq-agents-v2-hook-adapter.sh`, but on a real agents-v2 box it never does: the v2 runtime's shell-hook dispatcher (`agent/shell_hooks.py`) reads a `hooks:` block from `~/.hermes/config.yaml` that invokes the adapter, and the adapter merely *reads* `.claude/settings.json` to fan out to the classic `hook-gate.sh` hooks. On the v2.17 canary (`i-0277243ad3aed8109`) `settings.json` had 0 adapter refs / 94 `hook-gate.sh` refs while `~/.hermes/config.yaml` wired the adapter across 7 events, so the old grep could never fire. Signal 2 (`hq_runtime_config_wires_v2_adapter`) now requires BOTH the adapter installed under the tree at `.agents-v2-hooks/hq-agents-v2-hook-adapter.sh` AND the runtime config at `${HQ_HERMES_CONFIG_FILE:-~/.hermes/config.yaml}` invoking it. All three signals are still required, no non-agents-v2 host changes verdict, and every Claude-CLI path stays byte-identical. Regression coverage: `[19]` fixtures in `core/scripts/tests/hook-health-check.test.sh` rebuilt to mirror a real box — `settings.json` wiring the classic `hook-gate.sh` hooks (not the adapter), the adapter present under the tree, and `~/.hermes/config.yaml` wiring it via `HQ_HERMES_CONFIG_FILE`.

### Added — Skills
- **`/new-agent` — provision a fleet agent end-to-end.** One flow from "agent exists somewhere" to "verified capable for a defined job": resolves identity + membership, checks the company is cloud-backed, derives a capability manifest from the job (read-only by default, payment-key scope verified by prefix before sharing), issues secret/file grants with the field-tested CLI syntax (agent-email principal form, required `--permission`), generates a runtime bootstrap block for the layers grants can't reach (team-vault join, MCP registration), writes a mission brief into `companies/{co}/knowledge/agents/` (a synced path — unknown top-level dirs are silently excluded by sync), and gates "done" on the agent confirming a probe checklist from its own runtime. Repair mode diffs existing access and grants only gaps.
- **`/new-hire` — onboard a human teammate end-to-end.** Invite with role, groups-before-per-person-shares, least-privilege secret/file grants, an onboarding packet written into the team vault, an approval-gated welcome message, and follow-through until membership is active and the setup checklist is confirmed. Top-up mode brings an existing member's access up to their role without re-inviting.
...
### Changed — Skills
- **`/new-agent` documents the Agents v2 (create agent v2) branch.** Create mode now surfaces `--provider agents-v2` on `hq agents provision`, and a new "Agents v2 branch" section walks the two-phase delivery — provision → ready → `install-agents-v2-runtime` (the hq-pro operator script, dry-run by default, `--commit` to act, `plan|install|activate|rollback|status` with `--artifact-key`/`--sha256`) → the v2 probe checklist (unit active, `runtimeMode=agents-v2`, socket connected, heartbeat active, plus the legacy inbox watcher still active) and a live DM round-trip. Notes that the codex/grok brain CLI sign-in is a human step.
