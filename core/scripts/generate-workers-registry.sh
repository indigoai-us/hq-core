...
#
# It ships at assets/scaffold/core/scripts/generate-workers-registry.sh inside @indigoai-us/hq-cli and
# runs as the hidden command `hq core generate-workers-registry`. This file stays behind so every
# existing caller — skills, other scripts, CI, and muscle memory — keeps working
# against the path it already knows.
#
# Why the implementation moved: it is a derived-index generator invoked explicitly and by reindex, so
# process startup is immaterial and the CLI already owns the index primitives.
#
