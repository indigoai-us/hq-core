...
# environment stays stripped, but PATH shadowing alone NO LONGER controls which
# qmd the CLI runs: since hq-cli 5.94.1 package-local resolution works, so the
# CLI prefers its own bundled qmd and `$TMP/bin/qmd` is never invoked — the
# recording log is never written and every assertion below silently measures
# nothing. HQ_QMD_BIN is the documented override and takes precedence over both
# the bundled copy and PATH, so pin the stub explicitly.
