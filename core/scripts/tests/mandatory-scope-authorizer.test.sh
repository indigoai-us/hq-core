...
install_fixture() {
  ...
  mkdir -p "$TMP/.claude/hooks" "$TMP/core/scripts/lib" \
    "$TMP/companies/indigo/settings" "$TMP/companies/liverecover/settings" \
  ...
  touch "$TMP/companies/indigo/settings/.keep" "$TMP/companies/liverecover/settings/.keep"
...
payload='{"tool_name":"Read","session_id":"sess-bound","cwd":"'"$TMP"'","tool_input":{"file_path":"'"$TMP"'/companies/liverecover/settings/foo.yaml"}}'
rc="$(run_hook "$payload")"
...
install_fixture "indigo"
payload='{"tool_name":"Bash","session_id":"sess-bound","cwd":"'"$TMP"'","tool_input":{"command":"cat companies/liverecover/settings/secrets.yaml"}}'
rc="$(run_hook "$payload")"
