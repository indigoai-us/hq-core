...
# The first prompt of a session may carry the model-facing naming nudge
# (session-title-desktop-nudge.test.sh); what must be absent is a TITLE.
case "$out" in *sessionTitle*) fail "T6: the wrapper must emit no title for a stub, got '$out'" ;; esac
