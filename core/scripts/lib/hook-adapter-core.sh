...
hqad_fallback_records() {
  ...
  case "$tool" in
    Bash) ids="mandatory-scope-authorizer detect-secrets block-core-writes-bash block-policy-writes-bash block-hq-root-git-mutation block-unsafe-package-install" ;;
