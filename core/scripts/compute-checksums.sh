#!/usr/bin/env bash
# compute-checksums.sh — Regenerate SHA256 checksums for all locked paths in core/core.yaml
# Usage: core/scripts/compute-checksums.sh [path-to-core/core.yaml]
#
...
# core/core.yaml itself is excluded (self-referential / circular)
#
...
if [ ! -f "$CORE_YAML" ]; then
  echo "ERROR: core/core.yaml not found at $CORE_YAML" >&2
  exit 1
fi
...
yq '.rules.locked[]' "$CORE_YAML" | while IFS= read -r path; do
  # Skip core/core.yaml itself (circular reference)
  if [ "$path" = "core/core.yaml" ]; then
    continue
  fi
