...
#
# It ships at assets/scaffold/core/scripts/archive-old-threads.sh inside @indigoai-us/hq-cli and
# runs as the hidden command `hq core archive-old-threads`. This file stays behind so every
...
if ! command -v hq >/dev/null 2>&1; then
  echo "archive-old-threads.sh: requires the hq CLI — this script's implementation now ships with it." >&2
