...
#
# It ships at assets/scaffold/core/scripts/rebuild-workers-index.sh inside @indigoai-us/hq-cli and
# runs as the hidden command `hq core rebuild-index workers`. This file stays behind so every
...
if ! command -v hq >/dev/null 2>&1; then
  echo "rebuild-workers-index.sh: requires the hq CLI — this script's implementation now ships with it." >&2
  echo "Install it with: npm install -g @indigoai-us/hq-cli" >&2
  exit 127
fi

exec hq core --hq-root "$HQ_ROOT" rebuild-index workers "$@"
