...
async function main() {
  ...
  try {
    ...
    let eventKind = command === "start" ? "claim" : command === "report" ? "progress" : command;
    ...
    const decision = decideProgressPost({
      ...
      taskId: wantsStory ? storyId : "",
      taskStatus: wantsStory ? (patched?.status || storyStatus) : "",
      taskTitle: taskTitle || patched?.title,
    ...
    printResult(
      {
        ...
        posted: decision.post,
        skipReason: decision.post ? undefined : decision.reason,
