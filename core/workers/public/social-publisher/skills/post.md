...
1. **Load API key**: Read from `core/settings/post-bridge/.env` (the `POST_BRIDGE_API_KEY` var)

2. **Resolve account IDs**: Read `core/settings/post-bridge/config.json`. For the active profile + platform, get the Post-Bridge account ID:
...
```bash
POST_BRIDGE_API_KEY=$(grep POST_BRIDGE_API_KEY core/settings/post-bridge/.env | cut -d= -f2)
...
2. If 401: "API key invalid or expired. Check core/settings/post-bridge/.env"
