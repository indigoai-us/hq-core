...
`core/scripts/run-project.sh --dry-run {slug}` cleans up its ephemeral git worktree when it finishes, but **it does NOT delete the feature branch that was created alongside the worktree**. The branch (named per `prd.json.branchName`) persists in the target repo at whatever HEAD was active during the dry run.
...
**Fix at source:** Add a trap in `core/scripts/run-project.sh` dry-run path that runs `git branch -d "$branch"` after the worktree is removed, gated on the branch HEAD matching the checkout point (so real-run branches aren't touched).

## Related

- `core/policies/run-project-file-locks-stale.md` — related orchestrator cleanup hygiene
- `core/scripts/run-project.sh` — dry-run codepath (worktree cleanup but no branch cleanup)
