...
tags: [vendor:github, reference]
source: split-from-hq-github
---

## Rule

**On-demand reference — not auto-injected.** The one hard, always-on rule (pass an explicit `-R/--repo` to every resource-scoped `gh` command) lives in the `hq-github` gate. This file is the full soft reference: read it before deep GitHub work, or find a specific rule via `qmd get -c hq-infra hq-github-reference`.

Consolidated rules for working with github.com — the gh CLI, REST/GraphQL APIs, PR/review workflows, Actions, repo state flags, bot credentials, and library distribution via github-install.
