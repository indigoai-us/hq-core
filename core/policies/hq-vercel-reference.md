...
tags: [vendor:vercel, reference]
source: split-from-hq-vercel
---

## Rule

**On-demand reference — not auto-injected.** The one hard, always-on rule (never deploy to a production custom domain without explicit confirmation) lives in the `hq-vercel` gate. This file is the full soft reference for everything else; find a specific rule via `qmd get -c hq-infra hq-vercel-reference`.
