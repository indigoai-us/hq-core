...
This applies to `.claude/settings.local.json` specifically (the user-local merge partner). `.claude/settings.json` is core/core.yaml-locked and should not be edited mid-session for trials; use `settings.local.json` instead (see `hq-trial-hooks-stage-in-settings-local`).
