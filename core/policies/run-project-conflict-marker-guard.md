...
`validate_git_state()` in `core/scripts/run-project.sh` (and every swarm-merge auto-commit path) MUST scan the staged diff for unresolved merge-conflict markers before calling `git commit`. If any staged file contains a line matching `^(<{7}|={7}|>{7})([^<=>]|$)`, the orchestrator MUST:
...
- `core/scripts/run-project.sh` (main HQ)
