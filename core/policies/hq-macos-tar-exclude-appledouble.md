...
macOS `tar` uses Apple's `copyfile` extension to preserve extended attributes (Finder labels, quarantine flags, resource forks) by creating hidden `._filename` companion files. On non-macOS filesystems these have no meaning and pollute the target. Four `._*` files appeared at `s3://hq-deploy-{company}-assets/{project}/` from a tar push in a deck deploy session and required manual S3 cleanup.
