...
- **Backfill exists.** `core/scripts/backfill-company-skill-mirrors.sh` walks every company and ensures every top-level skill/command has its mirror. Safe to re-run.

### Prefix algorithm

When `/newcompany {slug}` runs, it computes `prefix` and writes it into the manifest entry:

1. Take the manifest key (e.g. `acme-corp`, `widgets`, `bravo-labs`)
2. Strip hyphens, lowercase (`acmecorp`, `widgets`, `bravolabs`)
3. Take the **first 3 characters** (`acm`, `wid`, `bra`)
4. If that collides with an existing prefix in the manifest → take **first 4 characters** (`acme`, `widg`, `brav`)
5. If that still collides → suffix `-2`, then `-3`, etc.

Example (4 companies, no collisions, all 3-char):
...
bash core/scripts/backfill-company-skill-mirrors.sh
```
...
1. Create a throwaway top-level company skill at `companies/{co}/skills/_test-bridge/SKILL.md` → confirm `.claude/skills/{prefix}-_test-bridge/` symlink appears, resolves to the source dir.
2. Attempt to Write directly to `.claude/skills/{prefix}-foo/SKILL.md` → blocked with stderr message; set `HQ_ALLOW_DIRECT_PREFIX_WRITE=1` and retry → passthrough.
3. Run `bash core/scripts/backfill-company-skill-mirrors.sh` twice → first run reports symlinks created, second run reports zero new symlinks (idempotent).
4. Confirm `/{prefix}-demo-hq` appears in the live skills list.
5. Worker-nested skill Write (e.g. `companies/{co}/workers/deploy/skills/deploy.md`) → no mirror created at `.claude/skills/{prefix}-deploy/` (out of scope by design).
...
- Backfill — `core/scripts/backfill-company-skill-mirrors.sh`
