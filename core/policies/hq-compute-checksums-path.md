...
The checksum script for HQ Core Governance is at `template/scripts/compute-checksums.sh` (not `core/scripts/compute-checksums.sh`). Requires `yq` (`brew install yq`). Run from the HQ repo root after modifying locked kernel files. Updates `template/core.yaml` checksums.
