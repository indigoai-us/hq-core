...
When `core/scripts/run-project.sh` finishes a project with all stories passing, the target repo's `.file-locks.json` is often left in a **modified-but-empty** state — the working tree shows it as `M` even though every `checkedOutBy` entry has been released. This is a stale-lock artifact from the final story's cleanup path.
...
**Fix at source:** `core/scripts/run-project.sh` final cleanup should `git checkout -- .file-locks.json` on completion if the file is tracked, or delete it if it isn't. Need to confirm which state the repo expects.

## Related

- `core/policies/run-project-progress-txt-no-commit-misleading.md` — sibling `progress.txt` artifact leak
- `core/policies/run-project-dry-run-branch-leak.md` — sibling orchestrator cleanup gap
- `core/scripts/run-project.sh` — orchestrator final cleanup path
