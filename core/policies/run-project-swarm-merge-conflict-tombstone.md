...
When `core/scripts/run-project.sh --swarm` merges a per-story worktree branch into the shared feature branch and the merge fails (conflict, hook failure, aborted merge, or any non-zero exit from `git merge`), the orchestrator MUST:
