...
For company AWS/prod work (e.g. deploying `hq-pro` to `hq-prod`), credentials come ONLY via `hq secrets exec` — agent sessions have NO local AWS-profile fallback. So a `NoCredentials` error means reach for the vault (`hq secrets exec`), never give up and never fall back to another company's profile.
