...
tags: [vendor:linear, reference]
source: split-from-hq-linear
---

## Rule

**On-demand reference — not auto-injected.** The three always-on hard rules (plain `Authorization` header, ≤255-char `description` → use `content`, and scan-checks-existing-PRDs) live in the `hq-linear` gate. This file is the full soft reference, including the soft batched-mutations guidance below.

Consolidated rules for working with Linear — covers authentication, GraphQL field semantics, batched mutations, and PRD sync hygiene.
