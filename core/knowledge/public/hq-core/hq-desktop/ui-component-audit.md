...
| `CompanyDetailView` | `company-detail.tsx` | Company detail showing core/settings/data/knowledge files | `companyId`, `onBack` |
...
| Component | File | Purpose | Key Props |
|-----------|------|---------|-----------|
| `WorkerCard` | `core/workers/worker-card.tsx` | Card for worker in registry view | `worker`, `onRun` |
| `WorkersRegistry` | `core/workers/index.tsx` | Full worker registry view | `onRunWorker` |
...
| **Usage** | Empire views (empire-view, workers-drill, companies-drill, projects-drill, worker-detail) | Dashboard views (overview, core/workers/worker-card, core/workers/index) |
