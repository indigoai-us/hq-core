...
| `list_workers` | Parse core/workers/registry.yaml | PARTIAL - missing fields |
...
| `get_hq_stats` | Count core/workers/projects/threads/checkpoints | PARTIAL - stats incomplete |
...
```yaml
- id: {company}-analyst
  path: core/workers/private/{company}-analyst/
  type: ResearchWorker
...
```
This looks for `core/workers/{worker_id}/` but the actual structure is:
- `core/workers/public/{worker_id}/` (standalone public workers)
- `core/workers/public/dev-team/{worker_id}/` (dev-team workers)
- `core/workers/private/{worker_id}/` (private workers)
...
2. **Fix `get_worker_detail`**: Fix path resolution to search `core/workers/public/`, `core/workers/public/dev-team/`, `core/workers/private/` using registry.yaml path field. Fix YAML parsing to read nested `worker.name`, `worker.type` and correct `skills` structure.
