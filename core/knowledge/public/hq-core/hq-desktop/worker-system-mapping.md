...
mcp:
  server:
    ...
    cwd: core/workers/public/dev-team/{worker-id}
  tools:
    - tool_name_1
    - tool_name_2
```
...
│     ├─ Read core/workers/registry.yaml → find worker path                 │
│     ├─ Read core/workers/{path}/worker.yaml → full config                 │
│     └─ Load context files listed in context.base                     │
│     │                                                                │
│     │  STATE: idle → loading → planning                              │
│     │                                                                │
│  3. Load skill definition                                            │
│     ├─ Format A: Read core/workers/{path}/skills/{skill}.md               │
...
| **Skill file content** | core/workers/{path}/skills/{skill}.md | **NOT PARSED** |
...
mcp:
  server:
    ...
    cwd: core/workers/public/dev-team/architect  # Working directory
```
...
```json
{
  ...
  "injected_to": "core/workers/public/dev-team/architect/worker.yaml",
...
core/workers/registry.yaml
  ...
  │           │     │
  │           │     └─ core/workers/{path}/skills/{skill}.md
