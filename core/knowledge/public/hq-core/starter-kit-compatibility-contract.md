...
- `core/workers/registry.yaml` has > 10 workers
- `workspace/orchestrator/state.json` exists
...
| **Knowledge browser** | Flat tree of `core/knowledge/` | Flat tree | Split public/private + company-scoped | Detect `core/knowledge/public/` vs flat |
...
| `core/knowledge/public/` (flat layout) | Show `core/knowledge/` contents directly as a flat tree. No public/private toggle. |
| `core/knowledge/private/` | Show only public knowledge. No private tab. |
...
| `core/workers/public/` + `core/workers/private/` split | Show visibility toggle/filter. |
| Flat `core/workers/` (no public/private split) | Show all workers in single list, no visibility filter. |
...
|---------|----------------------|-------------------|
| v5.3.0 | Codex workers + MCP pattern added to dev-team | Expect `core/workers/dev-team/codex-*` |
...
isTemplate =
  ...
  && !exists('core/workers/public/')   // template uses flat core/workers/
  && !exists('core/workers/private/')  // no visibility split
