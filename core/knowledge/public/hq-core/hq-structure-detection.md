...
| `core/knowledge/Ralph/` | dir | Ralph methodology (10 chapters) | Core knowledge |
| `core/knowledge/ai-security-framework/` | dir | Security best practices | Core knowledge |
| `core/packages/hq-pack-design-styles/` | dir | Design guidelines + swipes | Optional pack (install via `hq install @indigoai-us/hq-pack-design-styles`) |
...
| `core/workers/public/dev-team/` | dir | Shared dev-team workers | Copy + customize to author company-scoped workers |
...
| `core/workers/public/` | dir | Public workers (30+) | Flat `core/workers/` |
| `core/workers/private/` | dir | Private workers (10+) | Not in starter |
| `core/workers/public/dev-team/` | dir | 12-worker dev team | Not in starter |
| `core/workers/public/content-*` | dirs | Content team (5 workers) | Not in starter |
| `core/workers/public/pr-*` | dirs | PR team (5 workers) | Not in starter |
...
function detectHQInstance(path: string): HQDetectionResult {
  ...
  const required = {
    ...
    registryYaml: exists(path, 'core/workers/registry.yaml'),
  ...
  const features = {
    // Workers
    hasWorkers:          parseYaml(path, 'core/workers/registry.yaml').workers?.length > 1,
    workerVisibility:    isDir(path, 'core/workers/public') && isDir(path, 'core/workers/private'),
    workerCount:         parseYaml(path, 'core/workers/registry.yaml').workers?.length ?? 0,
...
| No `core/workers/` beyond sample | Show "Create your first worker" CTA |
| No `workspace/threads/*.json` | Show empty thread panel with "Run a session to see threads" |
| No `core/knowledge/public/` split | Show flat knowledge tree |
