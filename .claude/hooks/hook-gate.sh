...
is_in_minimal_profile() {
  case "$1" in
    block-hq-glob|block-hq-grep|warn-cross-company-settings|mandatory-scope-authorizer|detect-secrets|protect-core|block-core-writes|block-core-writes-bash|enforce-vault-write-access|route-company-skill-creation|validate-policy-frontmatter|cleanup-mcp-processes|block-unsafe-package-install|block-hq-root-git-mutation|block-foreground-timeout-over-harness-ceiling|block-hq-worktree-session|enforce-capability-link-render|enforce-humanize-before-send|session-title|surface-company-infra-policy)
      return 0
      ;;
    *)
      return 1
...
is_in_standard_profile() {
  case "$1" in
    block-hq-glob|block-hq-grep|warn-cross-company-settings|mandatory-scope-authorizer|detect-secrets|auto-checkpoint-trigger|auto-checkpoint-precompact|hq-autocommit|precompact-thrashing-detector|observe-patterns|block-inline-story-impl|screenshot-resize-trigger|protect-core|block-core-writes|block-core-writes-bash|enforce-vault-write-access|route-company-skill-creation|validate-policy-frontmatter|cleanup-mcp-processes|check-bridge-health|check-repo-active-runs|block-on-active-run|context-warning-50|checkpoint-stop-gate|inject-codex-checkpoint-reprompt|inject-local-context|auto-startwork|auto-session-project|native-plan-project-sync|rewrite-resume-sentinel|mirror-thread-to-company|inject-policy-on-trigger|natural-language-router|route-deep-plan-to-skill|block-builtin-plan-mode-during-deep-plan|block-plans-dir-during-deep-plan|journal-autocapture|journal-due|journal-precompact|purge-policy-ledger-precompact|load-journal-index-on-start|block-unsafe-package-install|check-hq-update|repair-stale-review-base|block-hq-root-git-mutation|block-foreground-timeout-over-harness-ceiling|block-hq-worktree-session|enforce-capability-link-render|enforce-humanize-before-send|session-title|surface-company-infra-policy|migrate-policy-triggers|hq-auto-acl-suggest|work-mesh-ground)
      return 0
      ;;
    *)
      return 1
