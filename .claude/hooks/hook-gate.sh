...
is_in_minimal_profile() {
  case "$1" in
    block-hq-glob|block-hq-grep|warn-cross-company-settings|mandatory-scope-authorizer|detect-secrets|protect-core|block-core-writes|block-core-writes-bash|enforce-vault-write-access|validate-policy-frontmatter|cleanup-mcp-processes|block-unsafe-package-install|block-hq-root-git-mutation|block-foreground-timeout-over-harness-ceiling|block-hq-worktree-session|enforce-capability-link-render|enforce-humanize-before-send|surface-company-infra-policy)
      return 0
      ;;
    *)
      return 1
