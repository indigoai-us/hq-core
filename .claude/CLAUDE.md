...
| Use secrets in dev | `/hq-secrets` — for repos with `.env.schema`, use `hq run -- <cmd>` (schema-driven, batch fetch, no `--only` list needed). Never wrap in `$(...)` or pipe — values may leak |
| Get a secret | `/hq-secrets` — for one-off use: `hq secrets exec --only KEY -- <cmd>`; to read metadata: `hq secrets get`; to reveal value: `hq secrets get --reveal` |
