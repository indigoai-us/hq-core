...
- **Find meeting notes, signals, or company context** → HQ-native first. Meeting notes live at `companies/{co}/sources/meetings/`; signals (decisions, action items, wins, risks, questions, commitments) at `companies/{co}/signals/`; entity context via the `ontology` skill. Read them with `/meeting-notes` and `/signals` (they resolve the active company and honor `companies/{co}/settings/knowledge/preferences.yaml`). If a store is empty or unactivated, steer the user to turnkey activation (`/designate-team` → HQ meeting bot) and note HQ Pro/billing is coming — **never** default to email or third-party notetakers (Fireflies/Otter/etc.). Detail: `core/knowledge/public/hq-core/native-knowledge-stores.md`.
...
- **Every git/gh *mutation* MUST carry its own explicit repo anchor in the same Bash call** — `git -C /abs/path <cmd>` or `gh … -R owner/repo` (`gh repo create` is self-anchoring: its target is named in its args; `--source`, if used, must be an absolute non-HQ path). Do **not** use `( cd /abs/path && git <cmd> )` as the anchor — the Claude Code harness silently strips a leading `cd <path> && ` when the path equals the session cwd (verified 2026-06-08, re-verified 2026-06-09), so the enforcement hook never sees it. Bare mutations whose effective cwd is the HQ root are **mechanically blocked** by `.claude/hooks/block-hq-root-git-mutation.sh` (PreToolUse Bash, all hook profiles); when the harness-reported cwd already resolves to a nested non-HQ repo, the hook's cwd fallback permits the (possibly harness-stripped) mutation, since it cannot land on HQ root. Rationale: HQ root is itself a git repo with every working repo nested under it, and shell cwd silently drifts across context compaction, long-running tools, and parallel-call leakage — so an earlier `cd` is **never** a safe anchor for a write. Mechanical backstop for hard policies `hq-root-never-push-remote` / `hq-git-discipline`; regression test `core/scripts/tests/block-hq-root-git-mutation.test.sh`. Sanctioned HQ-internal git work: prefix the single command with `HQ_ALLOW_HQ_ROOT_GIT=1`.
