...
**Rationale:** User correction — posted to `#team-product` in Voyage Slack without asking. Channel posts are visible to many people and should always be explicitly approved.

## Workspace routing

### `#hq` Slack channel is on the Indigo workspace
[from `hq-slack-channel-{company}-workspace.md`]
...
This global pointer ensures any session posting about HQ finds the workspace routing rule, while the canonical details live in the Indigo company policy where they belong.
