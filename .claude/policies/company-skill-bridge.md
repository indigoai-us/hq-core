...
1. Take the manifest key (e.g. `acmestudio`, `acme-mgmt`, `acme-content`)
2. Strip hyphens, lowercase (`acmestudio`, `acmemgmt`, `acmecontent`)
3. Take the **first 3 characters** (`acm`, `acm`, `acm`)
4. If that collides with an existing prefix in the manifest → take **first 4 characters** (`acme`, `acm2`, `acmc`)
5. If that still collides → suffix `-2`, then `-3`, etc.

Example for a 3-tenant manifest (no collisions):

```
acmestudio      acm     acmebrands          acm2
indigo          ind     acmemgmt            acm3
```

In a populated manifest with N tenants, each new `/newcompany` invocation walks rules 3–5 above against the existing prefix set.
