...
- `hq auth login` issues tokens for pool **`us-east-1_IksCYBcBr`** (client `4mmujmjq3srakdueg656b9m0mp`) — the canonical HQ Identity pool per `auth-strategy.md`
- The hq-deploy production CloudFormation template (`src/infra/cloudformation/template.yaml`) defaults to pool **`us-east-1_AXf6Kb5nE`** (client `7acei2c8v870enheptb1j5foln`) — a different pool

The deploy skill calls these "the shared HQ Identity pool" but they are NOT the same pool in production.
...
2. Invalidate CloudFront: `aws cloudfront create-invalidation --distribution-id E21VXOFJ95HKGU --paths "/{slug}/*"` (Distribution aliases: `{your-domain}.com`, `*.{your-domain}.com`)
...
**Long-term fix:** update `src/infra/cloudformation/template.yaml` to use pool `us-east-1_IksCYBcBr` / client `4mmujmjq3srakdueg656b9m0mp`. The 401 looks identical to an expired-token error because both SDK and server-side logs report "invalid token" rather than "wrong pool."
