...
**Neither Write nor Edit can carry the bypass.** The PreToolUse core-protect hook only observes env vars on Bash invocations; the same vars attached to a Write or Edit call are ignored and the edit is blocked. When the locked target is `scripts/`, `.claude/hooks/`, `.claude/CLAUDE.md`, or `.claude/settings.json`, write via Bash using one of:
