...
Captured 2026-04-27 during the trim-session-context plan. The 11 soft-enforcement Vercel policies (`hq-vercel-cli-binary-not-npx`, `hq-vercel-install-mutates-pkg-pin-peer-deps`, `hq-vercel-logs-streaming-empty-in-tool-context`, `hq-vercel-post-deploy-chunk-hash-probe`, `hq-vercel-reproduce-install-command-locally`, `vercel-domain-team-move`, `vercel-domain-transfer-reissues-verification`, `vercel-env-pull-environment`, `vercel-preview-sso`, `vercel-route53-domain-setup`, `vercel-scope-for-read-commands`) remain separate — they are already excluded from cold-start by the enforcement filter and don't add to the digest cost.
