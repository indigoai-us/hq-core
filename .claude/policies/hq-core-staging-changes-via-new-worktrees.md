...
created for that task, via the HQ `/worktree` skill — do not hand-roll
`git worktree add`:

```bash
bash personal/skills/worktree/worktree.sh \
  --name <task-slug> \
...
checkout (working tree, refs, stash) is untouched. Only if the skill is
...
- Fixing a hook script here → `bash personal/skills/worktree/worktree.sh
