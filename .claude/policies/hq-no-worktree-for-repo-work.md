...
NEVER create a git worktree or new branch in HQ when starting work on a project repo (VYG, {company}-{your-project}, etc.). HQ must stay on `main` at all times. All branching and worktree creation happens inside the target repo itself (e.g. `repos/private/vyg`). Each repo has its own branching strategy independent of HQ.
