...
During an applies_to backfill session, the post-compact summary asserted 38 tagged policies + scripts + hooks were pending commit. `git log --oneline -- scripts/backfill-policy-applies-to.sh` revealed the work had already landed in an earlier checkpoint commit (`e4eac4507`). Re-committing would have been a no-op at best; at worst it could have reintroduced stale state.
