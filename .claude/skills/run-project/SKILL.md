...
- **Carry the parent company into every detached lane** — before the shared launch block, resolve `HQ_SPAWN_COMPANY` with `hq-session.sh --session-id "$SID" get company_slug` and stop if it is empty; set optional `HQ_SPAWN_PROJECT` and `HQ_SPAWN_TASK` from the owning session metadata. The shared block exports those values with `HQ_PARENT_SESSION_ID`, so SessionStart binds the engine's child session rather than treating the parent `HQ_SESSION_ID` as the child. Never infer a company from cwd. The workflow runner also reads the parent session metadata when these environment values are missing, so a retry lane is not unbound.
