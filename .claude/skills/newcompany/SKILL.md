...
Codex adapter for `/newcompany`.

**Arguments:** [company-slug]

## Source Of Truth

Read `.claude/commands/newcompany.md` first. That slash command owns the workflow, flags, safety gates, file locations, and completion criteria. This skill exists so Codex can discover and execute the same HQ capability without duplicating the full command body.
...
- Create only the documented company scaffold and manifest/module/qmd entries.
- Do not collect or write secrets directly; route credential setup to the existing secrets workflow.
