...
- Session opened with an `<auto-conduct>` block → the mode is the HQ default
  ...
  `personal/settings/orchestrator.yaml` override). No engine is preset: unless
  the user named one, ask the engine question above and persist the answer.
  Then confirm the mode in one line. `/conduct off` still leaves it for the
  session.
...
- **The mode can be the HQ default.** `conduct.default_enabled` in
  ...
  `personal/settings/orchestrator.yaml`) makes every fresh session start in
  conduct mode via `.claude/hooks/auto-conduct.sh`. The engine is never
  preset; it is asked or named per session. Per session: `HQ_AUTO_CONDUCT=1|0`
