...
Codex adapter for `/checkpoint`.

**Arguments:** [task-id]

## Source Of Truth

Read `.claude/commands/checkpoint.md` first. That slash command owns the workflow, flags, safety gates, file locations, and completion criteria. This skill exists so Codex can discover and execute the same HQ capability without duplicating the full command body.
...
- Preserve checkpoint idempotency. If a recent auto-checkpoint exists, upgrade it instead of creating a duplicate.
- After writes, rebuild thread indexes with the existing scripts when the command calls for it.
