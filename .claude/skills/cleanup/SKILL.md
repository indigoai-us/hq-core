...
Codex adapter for `/cleanup`.

**Arguments:** [--audit | --migrate | --fix | --consolidate-learnings]

## Source Of Truth

Read `.claude/commands/cleanup.md` first. That slash command owns the workflow, flags, safety gates, file locations, and completion criteria. This skill exists so Codex can discover and execute the same HQ capability without duplicating the full command body.
...
- Default to audit/report mode unless the user explicitly asks for `--fix`, `--migrate`, or another write mode.
- For consolidation modes, show a concise proposed change set before any broad rewrite.
