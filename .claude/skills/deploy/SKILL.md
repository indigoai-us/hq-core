...
| `password` | Sensitive content, casual share over Slack/email, recipients unknown ahead of time. | App owner sets a password (Argon2id-hashed). Visitors land on `hq.indigo-hq.com/__access`, enter the password, get a 24h `hq-access` JWT cookie scoped to `.indigo-hq.com`. |
| `private` | Sensitive content, **known recipients by email/domain**, want individual revocation. | Visitors must be signed in to hq-auth (`auth.indigo-hq.com`) AND their email must be on the app's allowlist. Lands on `hq.indigo-hq.com/__private`, which checks the session + allowlist and mints the same `hq-access` JWT. |
...
if [ "$SENSITIVE" = "true" ] && [ "$ACCESS_MODE" = "private" ]; then
  ...
  echo "[deploy] private mode: $PATTERN_LIST can sign in via auth.indigo-hq.com to view." >&2
fi
```
...
> Live at `https://$APP_SUBDOMAIN.indigo-hq.com` — gated to {[EMAIL], @indigo.ai}. They'll sign in via auth.indigo-hq.com on first visit.
