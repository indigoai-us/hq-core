...
#                     HQ scripts/verify-promotion-clean.sh logic.
...
#   provenance        fail if any .claude/policies/*.md contains a literal
...
#   public-frontmatter
#                     fail if any changed .claude/policies/*.md lacks
...
#   core-yaml-locked  fail if core.yaml changed without label "version-bump".
#   special-case-files
...
mode="${1:-}"
if [[ -z "$mode" ]]; then
  echo "Usage: $0 <denylist|policy-rationale|slugs|users-path|provenance|vendor-public-ok|public-frontmatter|commands-skills-tripwire|core-yaml-locked|special-case-files|denylist-drift>" >&2
  exit 2
fi
...
case "$mode" in
  ...
  users-path)
    ...
    leaks=0
    surfaces=(.claude scripts knowledge/public)
  ...
  provenance)
    ...
    done < <(find .claude/policies -maxdepth 2 -type f -name '*.md' 2>/dev/null || true)
    if [[ $leaks -gt 0 ]]; then
